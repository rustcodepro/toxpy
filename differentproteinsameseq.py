...
def toxcompare_different_protein_same_sequences(gff_file1:str, gff_file2:str, fastafile1:str, fastafile2:str) -> str:
	"""
			...
			for (val, key) in protein_1.items():
				for (val2, key2) in protein_2.items():
					if id1[i] != id2[valrange] and  val != val2 and key == key2:
						...
						id2start = int(proteincoordinates2[valrange][0])
						id2end = int(proteincoordinates2[valrange][1])
	...
	with open("comparative-common.txt", 'w') as filewrite:
		...
		for i in range(len(common_proteins)):
			filewrite.write(str(common_proteins[i][0][0]) +'\t' + str(common_proteins[i][0][1]) +'\t' + str(common_proteins[i][0][2]) +'\t' + str(common_proteins[i][0][3]) +'\t' + str(common_proteins[i][1][4]) + '\t' + str(common_proteins[i][1][0]) +'\t'
	   + str(common_proteins[i][1][1]) +'\t' + str(common_proteins[i][1][2]) +'\t' + str(common_proteins[i][1][3]) + '\t' + str(common_proteins[i][1][4]) + '\n')
		filewrite.close()
	with open("differentprotein-same-seq.txt", 'w') as filecommon:
